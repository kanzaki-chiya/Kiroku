...
### ⭐️ 细粒度五维评分与深度评价
- **十分制总评**：支持 0.0 - 10.0 的个人独立总评分；
- **五维星级**：针对 **剧情**、**角色**、**演出**、**作画**、**音乐** 使用 5 星制、半星步进（档位信号，不计入总分）；
...
### 🔒 数据安全与自由备份
- **标准 JSON 备份**：一键导出包含个人记录、作品快照与分档配置的标准格式备份文件（format v2，不向前兼容旧版应用）；
- **智能增量导入**：导入时支持预览新增数量、重复条目与冲突项，提供跳过或覆盖策略；可导入 v1 十分制分项并折半为星级；
